...
// ─── SERVER-RELATED QUESTION IDs (auto N/A if no on-prem server) ──────────────
...
const DEFAULT_CATALOG = [
  ...
  // Entra ID (conditional — shown if client enrolled in Entra ID)
  {id:"eid1",category:"Entra ID",question:"Are all devices enrolled in Entra ID / Azure AD?",standard:"100% of managed devices enrolled; compliance policies applied.",weight:2,criticality:"Critical"},
  {id:"eid2",category:"Entra ID",question:"Is Conditional Access configured and enforced for all users?",standard:"Baseline CA policies active; MFA required for all cloud apps; legacy auth blocked.",weight:2,criticality:"Critical"},
  {id:"eid3",category:"Entra ID",question:"Are Conditional Access policies following Microsoft best practices?",standard:"Microsoft Secure Score CA recommendations implemented; named locations and risk policies configured.",weight:2,criticality:"High"},
  {id:"eid4",category:"Entra ID",question:"Is Entra ID Password Protection enabled?",standard:"Custom banned password list active; on-prem proxy deployed if hybrid.",weight:2,criticality:"High"},
  {id:"eid5",category:"Entra ID",question:"Are privileged roles using Entra ID PIM (Privileged Identity Management)?",standard:"No standing Global Admin assignments; PIM activated for all privileged roles.",weight:2,criticality:"Critical"},
  {id:"eid6",category:"Entra ID",question:"Is Entra ID Sign-in risk and user risk policy configured?",standard:"Sign-in risk policy requires MFA on medium+ risk; user risk policy enforces password change on high risk.",weight:2,criticality:"High"},
  {id:"eid7",category:"Entra ID",question:"Is seamless SSO configured for hybrid-joined devices?",standard:"Seamless SSO enabled for hybrid environments; SSPR deployed for self-service password reset.",weight:1,criticality:"Medium"},
  {id:"eid8",category:"Entra ID",question:"Are guest and external accounts reviewed regularly in Entra ID?",standard:"Guest access review conducted quarterly; unused guest accounts removed; external collaboration restrictions in place.",weight:2,criticality:"High"},
];
...
// ─── STORAGE ─────────────────────────────────────────────────────────────────
function lsGet(k){try{const v=localStorage.getItem("it8_"+k);return v?JSON.parse(v):null;}catch{return null;}}
function lsSet(k,v){try{localStorage.setItem("it8_"+k,JSON.stringify(v));}catch(e){console.error(e);}}
...
export default function App(){
  ...
  const persist=useCallback((cat,cli,asm,tch)=>{
    lsSet("catalog",cat??catalog);lsSet("clients",cli??clients);lsSet("assessments",asm??assessments);lsSet("techs",tch??techs);
    setSavedMsg("SAVED");setTimeout(()=>setSavedMsg(""),1500);
  },[catalog,clients,assessments,techs]);
              ...
              <div style={{display:"flex",gap:2}}>
                {navItems.map(n=>{const active=view===n.id;return(
                  <button key={n.id} onClick={()=>setView(n.id)} style={{padding:"8px 14px",borderRadius:6,background:active?"rgba(79,179,199,0.14)":"transparent",color:active?"#fff":"#94A3B8",border:"none",cursor:"pointer",fontFamily:FONT,fontSize:13,fontWeight:active?600:500,position:"relative"}}>
                    {n.label}{active&&<div style={{position:"absolute",left:14,right:14,bottom:-1,height:2,background:T.accentGlow}}/>}
                  </button>
                );})}
              </div>
            </div>
...
function AssessIcon(){return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 012-2V5a2 2 0 012-2h11"/></svg>;}
    ...
    <div style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden"}}>
      <div style={{padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          ...
          {s.grade&&<div style={{fontFamily:FONT,fontWeight:700,fontSize:20,color:gradeColor(s.grade),letterSpacing:-1}}>{s.grade}</div>}
        </div>
        {s.pct!==null?(
          <>
            <div style={{height:5,background:T.bg,borderRadius:999,overflow:"hidden",marginBottom:6}}>
        ...
        ):<div style={{fontFamily:FONT,fontSize:12,color:T.muted}}>No assessment yet</div>}
      </div>
      <div style={{display:"flex",borderTop:`1px solid ${T.border}`}}>
        <button onClick={()=>onStart(s.client)} style={{flex:1,padding:"11px 0",background:T.ok,border:"none",cursor:"pointer",fontFamily:FONT,fontSize:13,fontWeight:700,color:"#fff",borderBottomLeftRadius:s.pct?0:12,borderRadius:s.pct?"0 0 0 12px":"0 0 12px 12px"}}>
    ...
    return(
      <div style={{padding:"0 16px"}}>
        <div style={{padding:"20px 0 16px"}}>
          ...
          <div style={{fontFamily:FONT,fontSize:13,color:T.muted,marginTop:4}}>{assessed} of {clients.length} clients assessed · {p1total} critical findings open</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {summaries.map(s=><ClientCard key={s.client} s={s}/>)}
        </div>
      </div>
    );
  }
...
function IntakeModal({onConfirm,onCancel}){
  ...
  const Opt=({val,cur,set,label,color})=>(
    <button onClick={()=>set(val)} style={{flex:1,padding:"14px 8px",borderRadius:10,border:`2px solid ${cur===val?(color||T.accent):T.border}`,background:cur===val?(color||T.accent)+"18":"white",cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:15,color:cur===val?(color||T.accentInk):T.muted,transition:"all 0.12s"}}>{label}</button>
  );
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(11,23,41,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:16}}>
      <div style={{background:T.card,borderRadius:16,padding:"28px 28px 24px",maxWidth:420,width:"100%",boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        ...
        <div style={{marginBottom:24}}>
          ...
          {hasEntra===true&&<div style={{marginTop:8,padding:"8px 12px",background:T.okBg,borderRadius:8,fontFamily:FONT,fontSize:12,color:T.ok}}>Entra ID specific questions will be added to this assessment.</div>}
        </div>
        <div style={{display:"flex",gap:10}}>
...
function AssessView({clients,catalog,assessments,techs,activeClient,setActiveClient,isMobile,onSave}){
  ...
  const [intake,setIntake]=useState(null); // {hasServer, hasEntra}
  ...
  // Build effective catalog based on intake
  ...
  // Load or init assessment for this client
  useEffect(()=>{
    if(!client)return;
    ...
    if(last){
      ...
      setIntake(last.intake||{hasServer:true,hasEntra:false});
      setShowIntake(false);
    } else {
      setResponses({});
      setIntake(null);
  ...
  const autoSave=useCallback((newResponses,intakeData)=>{
    const hist=assessments[client]||[];
    const last=hist[hist.length-1];
    const entry={date:last?.date||new Date().toISOString(),assessor:"",responses:newResponses,intake:intakeData||intake,updatedAt:new Date().toISOString()};
    const filtered=hist.filter((_,i)=>i<hist.length-1);
    onSave({...assessments,[client]:[...filtered,entry]});
  },[assessments,client,intake,onSave]);

  const setResp=(id,field,val,skipSave)=>{
    setResponses(prev=>{
      const updated={...prev,[id]:{...(prev[id]||{}),[field]:val,...(field==="status"?{answeredAt:new Date().toISOString()}:{})}};
      if(!skipSave)autoSave(updated,null);
      return updated;
    });
  };
  ...
  const handleIntakeConfirm=(intakeData)=>{
    ...
    setShowIntake(false);
    // Auto N/A server questions if no server
    if(!intakeData.hasServer){
      setResponses(prev=>{
        const updated={...prev};
        SERVER_QS.forEach(qid=>{updated[qid]={...(updated[qid]||{}),status:"N/A",answeredAt:new Date().toISOString(),assessor:"System"};});
        autoSave(updated,intakeData);
        return updated;
      });
    } else {
      autoSave({},intakeData);
  ...
  const filteredQ=effectiveCatalog.filter(q=>activeCategory==="all"||q.category===activeCategory).filter(q=>!search||q.question.toLowerCase().includes(search.toLowerCase()));
  ...
  const TechSelect=({qid,value})=>(
    <select value={value||""} onChange={e=>setResp(qid,"assessor",e.target.value)} style={{fontFamily:FONT,fontSize:12,color:T.ink,border:`1px solid ${T.border}`,borderRadius:6,padding:"4px 8px",background:"white",cursor:"pointer",width:"100%"}}>
      ...
      return(
        <div style={{padding:"0 16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0 16px"}}>
            ...
            <div style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{answered}/{effectiveCatalog.length} answered</div>
          </div>
          <div style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,padding:"18px 16px",marginBottom:14}}>
            ...
            <div style={{fontFamily:FONT,fontWeight:600,fontSize:17,color:T.ink,lineHeight:1.35,marginBottom:6}}>{activeQ.question}</div>
            <div style={{fontFamily:FONT,fontSize:12,color:T.muted,lineHeight:1.5}}>{activeQ.standard}</div>
          </div>
          <div style={{marginBottom:14}}>
            <Eyebrow>Assessor</Eyebrow>
            <TechSelect qid={activeQ.id} value={activeR.assessor}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            {STATUSES.map(s=>{
              const isActive=activeR.status===s;const sc=statusColors[s][isActive?"active":"inactive"];
              return<button key={s} onClick={()=>setResp(activeQ.id,"status",isActive?null:s)} style={{padding:"18px 8px",borderRadius:12,border:`2px solid ${sc.border}`,background:sc.bg,color:sc.text,cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:15}}>{s}</button>;
            })}
          </div>
          ...
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"12px 14px",marginBottom:10}}>
            ...
            <textarea value={activeR.notes||""} onChange={e=>setResp(activeQ.id,"notes",e.target.value,true)} onBlur={()=>autoSave(responses,null)} placeholder="Add context…" rows={3} style={{width:"100%",fontFamily:FONT,fontSize:14,color:T.ink,border:"none",outline:"none",resize:"none",background:"transparent",boxSizing:"border-box",padding:"4px 0",marginTop:4}}/>
          </div>
          <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"12px 14px",marginBottom:14}}>
            ...
            <input value={activeR.evidence||""} onChange={e=>setResp(activeQ.id,"evidence",e.target.value,true)} onBlur={()=>autoSave(responses,null)} placeholder="https://…" style={{width:"100%",fontFamily:MONO,fontSize:13,color:T.accentInk,border:"none",outline:"none",background:"transparent",boxSizing:"border-box",padding:"4px 0"}}/>
          </div>
          <div style={{display:"flex",gap:10}}>
      ...
      <div>
        <div style={{padding:"12px 16px",background:T.card,borderBottom:`1px solid ${T.border}`,position:"sticky",top:56,zIndex:50}}>
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
            ...
            <button onClick={()=>setShowIntake(true)} style={{background:T.navyAlt,color:"#94A3B8",border:"none",borderRadius:8,padding:"9px 12px",cursor:"pointer",fontFamily:MONO,fontSize:10,fontWeight:600}}>INTAKE</button>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            <span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>PROGRESS</span>
            ...
            return(
              <div key={q.id} onClick={()=>{setActiveQId(q.id);setMobileDetail(true);}} style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,borderLeft:`4px solid ${sc}`,padding:"12px 14px",cursor:"pointer"}}>
                <div style={{display:"flex",gap:8,marginBottom:5,alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}><CritBadge kind={q.criticality}/><span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{q.category.split(" ")[0].toUpperCase()}</span></div>
                ...
                {r.assessor&&<div style={{fontFamily:MONO,fontSize:10,color:T.muted,marginTop:4}}>{r.assessor}</div>}
              </div>
            );
          })}
        </div>
  ...
  // ── DESKTOP 3-PANE ──
  return(
    <div style={{display:"grid",gridTemplateColumns:"220px minmax(0,360px) 1fr",gap:0,height:"calc(100vh - 112px)",background:T.card,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden",minWidth:0}}>
      {/* Left */}
      <div style={{borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",background:T.bg,height:"calc(100vh - 112px)",overflow:"hidden"}}>
        ...
        <div style={{padding:"10px 16px",borderBottom:`1px solid ${T.border}`}}>
          ...
          {intake&&<div style={{marginTop:8,display:"flex",gap:6}}>
            <span style={{fontFamily:MONO,fontSize:9,padding:"2px 6px",borderRadius:4,background:intake.hasServer?T.okBg:T.errBg,color:intake.hasServer?T.ok:T.err,fontWeight:600}}>{intake.hasServer?"ON-PREM":"CLOUD ONLY"}</span>
            {intake.hasEntra&&<span style={{fontFamily:MONO,fontSize:9,padding:"2px 6px",borderRadius:4,background:T.accentBg,color:T.accentInk,fontWeight:600}}>ENTRA ID</span>}
          </div>}
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"8px"}}>
          {[{id:"all",label:"All Questions"},...categories.map(c=>({id:c,label:c}))].map(cat=>(
            <button key={cat.id} onClick={()=>setActiveCategory(cat.id)} style={{width:"100%",display:"flex",justifyContent:"space-between",padding:"7px 10px",borderRadius:7,border:"none",cursor:"pointer",textAlign:"left",background:activeCategory===cat.id?T.accentBg:"transparent",color:activeCategory===cat.id?T.accentInk:T.ink2}}>
              <span style={{fontFamily:FONT,fontSize:13,fontWeight:activeCategory===cat.id?600:400}}>{cat.label}</span>
              <span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{effectiveCatalog.filter(q=>cat.id==="all"||q.category===cat.id).filter(q=>responses[q.id]?.status).length}/{effectiveCatalog.filter(q=>cat.id==="all"||q.category===cat.id).length}</span>
            </button>
          ))}
        </div>
        <div style={{padding:10,borderTop:`1px solid ${T.border}`}}>
          <button onClick={()=>setShowIntake(true)} style={{width:"100%",background:T.navyAlt,color:"#94A3B8",border:"none",borderRadius:7,padding:"8px 0",cursor:"pointer",fontFamily:MONO,fontSize:10,fontWeight:600,letterSpacing:0.5}}>RE-RUN INTAKE</button>
        </div>
      </div>
      ...
      <div style={{borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",height:"calc(100vh - 112px)",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          {filteredQ.map(q=>{
            ...
            return(
              <div key={q.id} onClick={()=>setActiveQId(q.id)} style={{display:"flex",gap:10,padding:"11px 14px 11px 12px",cursor:"pointer",borderLeft:`3px solid ${isActive?T.accent:sc}`,borderBottom:`1px solid ${T.border}`,background:isActive?"#fff":T.card}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",gap:6,marginBottom:3,alignItems:"center"}}><CritBadge kind={q.criticality}/><span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{q.category.split(" ")[0].toUpperCase()}</span></div>
                  <div style={{fontFamily:FONT,fontSize:13,color:T.ink,lineHeight:1.35,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{q.question}</div>
      ...
      {/* Right detail */}
      <div style={{overflowY:"auto",padding:"24px 28px 60px",height:"calc(100vh - 112px)",boxSizing:"border-box"}}>
        {!activeQ?(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",color:T.muted,fontFamily:FONT,gap:8}}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={T.border} strokeWidth="1.5"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
          ...
          <>
            <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"center",justifyContent:"space-between"}}>
              <div style={{display:"flex",gap:8,alignItems:"center"}}><CritBadge kind={activeQ.criticality}/><span style={{fontFamily:MONO,fontSize:10,color:T.muted}}>{activeQ.category.toUpperCase()}</span></div>
              {activeR.status&&<span style={{fontFamily:MONO,fontSize:11,fontWeight:600,padding:"3px 10px",borderRadius:999,background:activeR.status==="Complete"?T.okBg:activeR.status==="Partial"?T.warnBg:activeR.status==="Missing"?T.errBg:T.naBg,color:activeR.status==="Complete"?T.ok:activeR.status==="Partial"?T.warn:activeR.status==="Missing"?T.err:T.na}}>{activeR.status}</span>}
            </div>
            <h2 style={{fontFamily:FONT,fontWeight:600,fontSize:19,color:T.ink,lineHeight:1.35,marginBottom:6}}>{activeQ.question}</h2>
            <p style={{fontFamily:FONT,fontSize:13,color:T.muted,lineHeight:1.55,marginBottom:18}}>{activeQ.standard}</p>
            {/* Assessor */}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:16}}>
              {STATUSES.map(s=>{
                const isActive=activeR.status===s;const sc=statusColors[s][isActive?"active":"inactive"];
                return<button key={s} onClick={()=>setResp(activeQ.id,"status",isActive?null:s)} style={{padding:"13px 4px",borderRadius:10,border:`1.5px solid ${sc.border}`,background:sc.bg,color:sc.text,cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:13,transition:"all 0.1s"}}>{s}</button>;
              })}
            </div>
            {activeR.answeredAt&&<div style={{fontFamily:MONO,fontSize:10,color:T.muted,marginBottom:14}}>ANSWERED · {fmtTime(activeR.answeredAt)}</div>}
            {/* Notes */}
            <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,padding:"12px 14px",marginBottom:10}}>
              <Eyebrow>Notes</Eyebrow>
              <textarea value={activeR.notes||""} onChange={e=>setResp(activeQ.id,"notes",e.target.value,true)} onBlur={()=>autoSave(responses,null)} placeholder="Add context, findings, or observations…" rows={4} style={{width:"100%",fontFamily:FONT,fontSize:13,color:T.ink,lineHeight:1.5,border:"none",outline:"none",resize:"vertical",background:"transparent",boxSizing:"border-box",padding:"4px 0",marginTop:4}}/>
            </div>
            {/* Evidence */}
  ...
  return(
    <div>
      <div style={{display:"flex",gap:12,alignItems:"center",margin:`${isMobile?"12px":0} ${isMobile?"16px":"0"} ${isMobile?"12px":"24px"}`}}>
        ...
        <button onClick={handleScreenshot} disabled={!last||capturing} style={{display:"flex",alignItems:"center",gap:7,padding:"8px 16px",background:last?T.navy:"#e2e8f0",color:last?"#E8EDF5":T.muted,border:"none",borderRadius:8,cursor:last?"pointer":"default",fontFamily:FONT,fontWeight:600,fontSize:13,whiteSpace:"nowrap",flexShrink:0}}>
      ...
      {!last?(
        ...
        <div ref={dashRef} style={{background:"#fff",borderRadius:isMobile?0:14,border:isMobile?"none":`1px solid ${T.border}`,overflow:"hidden"}}>
          {/* Navy header */}
          <div style={{background:T.navy,color:"#E8EDF5",padding:`16px ${px} 20px`,borderBottom:`1px solid ${T.navyEdge}`}}>
            ...
            {last.assessor&&<div style={{fontFamily:FONT,fontSize:12,color:"#94A3B8",marginTop:4}}>Assessed · {new Date(last.date||last.updatedAt).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}</div>}
          </div>
              ...
              <div style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden"}}>
                ...
                {isMobile?(
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontFamily:FONT,fontSize:13}}>
                    <thead><tr style={{background:"#FAFBFC"}}>{["Priority","Category","Finding","Status","Assessor","Notes"].map(h=><th key={h} style={{textAlign:"left",padding:"10px 14px",fontFamily:MONO,fontSize:10,fontWeight:600,color:T.muted,letterSpacing:0.5,textTransform:"uppercase",borderBottom:`1px solid ${T.border}`}}>{h}</th>)}</tr></thead>
                    <tbody>{remItems.map((r,i)=>(
                      <tr key={i} style={{borderBottom:i<remItems.length-1?`1px solid ${T.border}`:"none"}}>
                        ...
                        <td style={{padding:"11px 14px",color:T.muted,fontSize:12,whiteSpace:"nowrap"}}>{r.assessor||"—"}</td>
  ...
  const stats=techList.map(tech=>{
    ...
    activeClients.forEach(client=>{
      ...
      hist.forEach(assessment=>{
        Object.values(assessment.responses||{}).forEach(r=>{
          if(r.assessor===tech&&r.answeredAt&&new Date(r.answeredAt)>cutoff){
            total++;clientCount++;
          }
        });
      });
      if(clientCount>0)perClient[client]=clientCount;
    });
    const allAnswers=clients.flatMap(client=>(assessments[client]||[]).flatMap(a=>Object.values(a.responses||{}).filter(r=>r.assessor===tech&&r.answeredAt).map(r=>r.answeredAt)));
    const lastActive=allAnswers.length>0?allAnswers.sort().reverse()[0]:null;
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:24}}>
        {stats.map(s=>(
          <div key={s.tech} style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,padding:"16px 18px"}}>
            ...
            <div style={{fontFamily:MONO,fontSize:36,fontWeight:600,color:s.total>0?T.accent:T.muted,letterSpacing:-1,lineHeight:1}}>{s.total}</div>
            <div style={{fontFamily:FONT,fontSize:12,color:T.muted,marginTop:2}}>questions answered</div>
            {s.lastActive&&<div style={{fontFamily:MONO,fontSize:10,color:T.muted,marginTop:8,letterSpacing:0.3}}>LAST · {fmtTime(s.lastActive)}</div>}
          </div>
        ))}
      </div>
...
function ManageView({catalog,techs,isMobile,onSave}){
  ...
  const [newQ,setNewQ]=useState({category:"",question:"",standard:"",weight:1,criticality:"Medium",defaultAssessor:""});
  ...
  const addQ=()=>{
    ...
    setNewQ({category:"",question:"",standard:"",weight:1,criticality:"Medium",defaultAssessor:""});
        ...
        <div style={{background:T.accentBg,borderRadius:12,padding:16,marginBottom:16,border:`1px solid #B6D7DD`}}>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <select value={newQ.defaultAssessor} onChange={e=>setNewQ({...newQ,defaultAssessor:e.target.value})} style={{flex:1,border:`1px solid ${T.border}`,borderRadius:8,padding:"9px 12px",fontSize:14,fontFamily:FONT}}>
                <option value="">Default assessor (optional)</option>
                {techs.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div style={{display:"flex",gap:8}}>
            ...
            {editId===q.id?(
              <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  ...
                  <select value={q.defaultAssessor||""} onChange={e=>update(q.id,"defaultAssessor",e.target.value)} style={{border:`1px solid ${T.border}`,borderRadius:6,padding:"6px 10px",fontSize:13,fontFamily:FONT}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",gap:6,marginBottom:4,alignItems:"center",flexWrap:"wrap"}}>
                  ...
                  {q.defaultAssessor&&<span style={{fontFamily:MONO,fontSize:10,color:T.accentInk,padding:"1px 6px",background:T.accentBg,borderRadius:4}}>{q.defaultAssessor}</span>}
                </div>
...
function ClientsView({clients,assessments,catalog,isMobile,onSave,onStart,onDashboard}){
  ...
  const [dirty,setDirty]=useState(false);
  const summaries=items.map(c=>{const hist=assessments[c]||[];const last=hist[hist.length-1];if(!last)return{client:c,pct:null,grade:null};return{client:c,...calcScore(last.responses,catalog),date:last.date||last.updatedAt};});
        ...
        {summaries.map(s=>(
          <div key={s.client} style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden"}}>
            <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:FONT,fontWeight:600,fontSize:15,color:T.ink}}>{s.client}</div>
                <div style={{fontFamily:MONO,fontSize:11,color:T.muted,marginTop:2}}>{s.pct!==null?`${s.pct}% · Last assessed ${new Date(s.date).toLocaleDateString()}`:"No assessment yet"}</div>
              </div>
            ...
            <div style={{display:"flex",borderTop:`1px solid ${T.border}`}}>
              <button onClick={()=>onStart(s.client)} style={{flex:1,padding:"10px 0",background:T.ok,border:"none",cursor:"pointer",fontFamily:FONT,fontSize:13,fontWeight:700,color:"#fff",borderRadius:"0 0 0 12px"}}>Assess</button>
              {s.pct&&<button onClick={()=>onDashboard(s.client)} style={{flex:1,padding:"10px 0",background:"transparent",border:"none",borderLeft:`1px solid ${T.border}`,cursor:"pointer",fontFamily:FONT,fontSize:13,fontWeight:600,color:T.muted,borderRadius:"0 0 12px 0"}}>Dashboard</button>}
            </div>
          </div>
        ))}
      </div>
...
// ─── TECHS VIEW (accessible via settings, not main nav) ──────────────────────
function TechsView({techs,onSave,isMobile}){
  const [items,setItems]=useState(techs);
  const [newT,setNewT]=useState("");
  const [dirty,setDirty]=useState(false);
  const add=()=>{if(newT.trim()&&!items.includes(newT.trim())){setItems(p=>[...p,newT.trim()]);setNewT("");setDirty(true);}};
  return(
    <div style={{padding:isMobile?"0 16px":"0",maxWidth:480}}>
      <div style={{margin:"16px 0 20px"}}><div style={{fontFamily:FONT,fontWeight:700,fontSize:20,color:T.ink}}>Technicians</div><div style={{fontFamily:FONT,fontSize:13,color:T.muted,marginTop:2}}>Manage the assessor list used across all assessments</div></div>
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <input value={newT} onChange={e=>setNewT(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Add technician name…" style={{flex:1,border:`1px solid ${T.border}`,borderRadius:8,padding:"10px 14px",fontSize:14,fontFamily:FONT}}/>
        <button onClick={add} style={{background:T.navy,color:"white",border:"none",borderRadius:8,padding:"10px 18px",cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:14}}>Add</button>
        {dirty&&<button onClick={()=>{onSave(items);setDirty(false);}} style={{background:T.ok,color:"white",border:"none",borderRadius:8,padding:"10px 16px",cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:14}}>Save</button>}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {items.map(t=>(
          <div key={t} style={{background:T.card,borderRadius:10,padding:"13px 16px",border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontFamily:FONT,fontWeight:600,fontSize:15,color:T.ink}}>{t}</span>
            <button onClick={()=>{setItems(p=>p.filter(x=>x!==t));setDirty(true);}} style={{background:T.errBg,color:T.err,border:`1px solid ${T.errBorder}`,borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:13,fontFamily:FONT,fontWeight:600}}>Remove</button>
          </div>
        ))}
      </div>
    </div>
