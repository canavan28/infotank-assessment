  ...
  return (
    <div style={{ fontFamily: FONT, minHeight: "100vh", background: T.bg, color: T.ink, maxWidth: "100vw", overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: isMobile ? "100%" : 1400, margin: "0 auto", padding: isMobile ? "16px 0 80px" : "28px 24px" }}>
        ...
        {view === "assess" && <AssessView clients={clients} catalog={catalog} assessments={assessments} techs={techs} activeClient={activeClient} setActiveClient={setActiveClient} isMobile={isMobile} onSave={saveAssessments} user={user} />}        {view === "dashboard" && <DashboardView clients={clients} assessments={assessments} catalog={catalog} activeClient={activeClient} setActiveClient={setActiveClient} isMobile={isMobile} />}
...
// ─── ASSESS VIEW ──────────────────────────────────────────────────────────────
function AssessView({clients,catalog,assessments,techs,activeClient,setActiveClient,isMobile,onSave,user}){  const [client, setClient] = useState(activeClient || clients[0]);
