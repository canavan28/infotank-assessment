  ...
  return(
    <div style={{fontFamily:FONT,minHeight:"100vh",background:T.bg,color:T.ink,maxWidth:"100vw",overflowX:"hidden"}}>
      ...
      <div style={{maxWidth:isMobile?"100%":1400,margin:"0 auto",padding:isMobile?"16px 0 80px":"28px 24px"}}>
        ...
        {view==="dashboard"&&<DashboardView clients={clients} assessments={assessments} catalog={catalog} activeClient={activeClient} setActiveClient={setActiveClient} isMobile={isMobile} user={user} onSave={saveAssessments}/>}
            ...
            return(
              <div key={q.id} onClick={()=>{setActiveQId(q.id);setMobileDetail(true);}} style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,borderLeft:`4px solid ${sc}`,padding:"12px 14px",cursor:"pointer"}}>
                <div style={{display:"flex",gap:6,marginBottom:5,alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{display:"flex",gap:5,alignItems:"center"}}><CritBadge kind={q.criticality}/><TypeBadge t={q.remedType}/>{q.onsite&&<OnsiteBadge/>}</div>
        ...
        <div style={{background:T.accentBg,borderRadius:12,padding:16,marginBottom:16,border:`1px solid #B6D7DD`}}>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <select value={newQ.onsite||false} onChange={e=>setNewQ({...newQ,onsite:e.target.value==="true"})} style={{flex:1,border:`1px solid ${T.border}`,borderRadius:8,padding:"9px 10px",fontSize:14,fontFamily:FONT}}><option value="false">Remote OK</option><option value="true">Onsite Required</option></select>
            </div>
            <div style={{display:"flex",gap:8}}>
            ...
            {editId===q.id?(
              <div style={{flex:1,display:"flex",flexDirection:"column",gap:8}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  ...
                  <select value={q.onsite||false} onChange={e=>update(q.id,"onsite",e.target.value==="true")} style={{border:`1px solid ${T.border}`,borderRadius:6,padding:"6px 9px",fontSize:13,fontFamily:FONT}}><option value="false">Remote OK</option><option value="true">Onsite Required</option></select>
